...
const useAuthController = () => {
	...
	const signUp = async (alert, data, setErrorLogList, requestAuth = false) => {
		try {
			...
			console.log(error.errorLogList);
	...
	const refresh = async (accessToken) => {
		try {
			if (accessToken) {
				...
				dispatch(authActions.setToggleLoadedAuth({ toggle: true }));
				throw new ErrorService(response.data.errorName, response.data.message, response.data.errorList);
			}
			dispatch(authActions.setToggleLoadedAuth({ toggle: true }));
		} catch (error) {
			return {
	...
	return { openAuthModal, closeAuthModal, setAuthFormType, setToggleRequestAuth, signUp, signIn, signOut, refresh };
};
